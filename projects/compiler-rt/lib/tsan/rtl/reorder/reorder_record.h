...
PACKED_STRUCT(ORDEREvent) {
  static const u8 TYPE_INDEX = EventType::Order;
  const u8 type_index = TYPE_INDEX;
  u64 pc : 48;
  u32 orderType : 32;
  u64 order : 48;
  ...
  char file[10];
  ALWAYS_INLINE
  explicit ORDEREvent(u64 pc , u32 o, u64 ord, u32 l,  char *f)
      :  pc(pc), orderType(o), order(ord), line(l){
    my_strncpy(file, f, sizeof(file));
  }
};
static_assert(sizeof(ORDEREvent) == 31, "compact struct (align 8) not supported, please use clang 3.8.1");
...
void enter_func(__tsan::ThreadState *thr, uptr orderType);
