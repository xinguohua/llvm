...
void impl_order(ThreadState *thr, uptr pc, u32 orderType, u32 line, char* file) {
  int tid = thr->tid;
  MC_STAT(thr, c_func_call)
  auto& buf = uctx->tlbufs[tid];
  buf.put_event(ORDEREvent((u64)pc, (u32)orderType, (u64)(__sync_fetch_and_add(&uctx->order, 1)), line, file));
  buf.last_fe = buf.e_counter_;
}
